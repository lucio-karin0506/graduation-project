...
def draw_fast_stoch(df, fastk_period, fastd_period, price1, price2, price3):
    copy_df = copy.deepcopy(df)
    copy_df.reset_index(inplace=True)

    indicator.add_STOCHF(copy_df, fastk_period, fastd_period, price1, price2, price3)
    ...
    ax_sub.set_title('Fast Stochastic', fontsize=15)
    # copy_df['fastk_' + str(fastk_period) + '_' + str(fastd_period)].iloc[0] = 0
    ax_sub.plot(xdate, copy_df['fastk_' + str(fastk_period) + '_' + str(fastd_period)], label='fastk' + str(fastk_period))
    ax_sub.plot(xdate, copy_df['fastd_' + str(fastk_period) + '_' + str(fastd_period)], label='fastd' + str(fastd_period))
...
if __name__ == "__main__":
    ...
    draw_fast_stoch(df, 5, 3, 'high', 'low', 'close')