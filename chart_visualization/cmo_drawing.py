...
def draw_cmo(df, period, price):
    copy_df = copy.deepcopy(df)
    copy_df.reset_index(inplace=True)

    indicator.add_CMO(copy_df, period, price)
    print(copy_df)
    ...
    # ax_sub에 rsi 차트 그림
    ax_sub.set_title('CMO', fontsize=15)
    # copy_df['cmo_'+str(period)].iloc[0] = 0
    ax_sub.plot(xdate, copy_df['cmo_' + str(period)], label='CMO' + str(period))
