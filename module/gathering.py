...
class Gathering:
    ...
    def get_stock(self, code, startdate, enddate='today', interval='D'):
    ...
    def _get_week_stock(self, code, startdate, enddate):
        ...
        try:
            ...
            if datetime.strptime(startdate, '%Y-%m-%d').date() < datetime.strptime('2017-10-10', '%Y-%m-%d').date() \
                ...
                df['Date'] = pd.to_datetime(df['Date'])  # 데이터프레임의 날짜형식을 변환하기 위함
                df.loc[df[df.Date == '2017-10-09'].index[0], 'Date'] = datetime.strptime('2017-10-10',
                                                                                         '%Y-%m-%d')  # 날짜를 변경 9일 -> 10일
                df.set_index('Date', inplace=True)
...
if __name__ == "__main__":
    mod = Gathering()
    df1 = mod.get_stock('000660', '1990-01-01', interval='w')
