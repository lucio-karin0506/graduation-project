from p407_gui.module import core
import talib as ta
...
def sma_cross(df, short=5, long=20, window_size=0, target='close'):
    ...
    if core.is_df(df) & core.is_posint([short, long], zero=False) & core.is_posint(window_size,
                                                                                   zero=True) & core.is_dflen(df,
                                                                                                              [short,
                                                                                                               long]) & core.is_column(
            df, target):
        df.reset_index(inplace=True)
        if isinstance(target, str):
            target = [target]
        for tar in target:
            c_name = f'ma_cross_{short}_{long}_{window_size}({tar})'
            df[c_name] = 0

            short_ma = ta.MA(df[tar], timeperiod=short)
            long_ma = ta.MA(df[tar], timeperiod=long)
            gold, dead = core.ws_cross(core.cross_up(short_ma, long_ma), core.cross_down(short_ma, long_ma),
                                       window_size)
...
def dema_cross(df, short=5, long=20, window_size=0, target='close'):
...
def vwma_cross(df, short=5, long=20, window_size=0, target='close'):
...
def macd(df, short=12, long=26, target='close'):
    ...
    if core.is_df(df) & core.is_posint([short, long], zero=False) & core.is_dflen(df, [short, long]) & core.is_column(
            df, target):
        if isinstance(target, str):
            target = [target]
        for tar in target:
...
def macd_cross(df, short=12, long=26, signal=9, window_size=0, target='close'):
    ...
    if core.is_df(df) & core.is_posint([short, long, signal], zero=False) & core.is_posint(window_size,
                                                                                           zero=True) & core.is_dflen(
            df, [short, long, signal]) & core.is_column(df, target):
        df.reset_index(inplace=True)
        if isinstance(target, str):
            target = [target]
        for tar in target:
...
def bbands(df, period=20, multid=2, target='close'):
    ...
    #####################################################
    if core.is_df(df) & core.is_posint(period, zero=False) & core.is_pos(multid, zero=False) & core.is_column(df,
                                                                                                              target):
        if isinstance(target, str):
            target = [target]
        for tar in target:
...
def stochf(df, fastk_period=5, fastd_period=3, window_size=0):
    ...
    #####################################################
    if core.is_df(df) & core.is_posint([fastk_period, fastd_period], zero=False) & core.is_posint(window_size,
                                                                                                  zero=True) & core.is_dflen(
            df, [fastk_period, fastd_period]):
        df.reset_index(inplace=True)
...
def stoch(df, fastk_period=5, slowk_period=3, slowd_period=3, window_size=0):
    ...
    #####################################################
    if core.is_df(df) & core.is_posint([fastk_period, slowk_period, slowd_period], zero=False) & core.is_posint(
            window_size, zero=True) & core.is_dflen(df, [fastk_period, slowk_period, slowd_period]):
        df.reset_index(inplace=True)
...
def make_label(df, RRPB=0.03, RFPT=0.02, TBR=0.003, BBR=0.003, target='close'):
    ...
    if core.is_df(df) & core.is_column(df, target) & core.is_pos([RRPB, RFPT, TBR, BBR], zero=True):
        ...
        for label_target in target:
            ...
            df.drop(['local_max', 'local_min', 'top', 'bottom', 'top_zone', 'bottom_zone'], axis='columns',
                    inplace=True)
