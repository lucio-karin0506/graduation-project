...
'''
다이얼로그
1. MACD 크로스 레이블 파라미터 설정 다이얼로그
'''
class macd_cross_label_Param(QDialog):
    def __init__(self, title, path, parent):
        ...
        self.signal_period_label = QLabel('MACD 지수이동평균', self)
        self.signal_period_edit = QLineEdit(self)
        self.signal_period_edit.setPlaceholderText('9')
        ...
        hlay4.addWidget(self.signal_period_label)
        hlay4.addWidget(self.signal_period_edit)
    ...
    def confirmIt(self):
        ...
        gathering_info = {'df': df,
                          ...
                          'signal': int(self.signal_period_edit.text()),
                          'target': str(self.target_option.currentData())
                          }

        label_indicator.macd_cross(gathering_info['df'], gathering_info['short'], gathering_info['long'],
                                   gathering_info['signal'], gathering_info['target'])
        ...
        if msg == QMessageBox.Yes:
            macd_cross_label_Param.close(self)
        # 2. 그래프 생성
        # 3. 지표 리스트에 지표 목록 생성
