...
from p407_gui.interface import (stock_chart_menu, simple_strategy_menu,
                                basic_backtest_menu, label_backtest_menu, comprehensive_chart_menu,
                                stock_filtering_menu, debug_log)
...
'''
전체 GUI 화면
1. 메뉴 바(종목차트, 거래전략(단순전략, 복합전략), 백테스트(기본, 레이블링), 종합차트, 종목필터링)
2. 탭 호스트
'''
class MainWindow(QMainWindow):
    ...
    def createMenus(self):
        ...
        newOrder = QMenu('&생성', self)
        newOrder.setStatusTip('생성')
        simpleStrategy = QAction('단순전략 생성', self)
        multiStrategy = QAction('복합전략 생성', self)
        simpleStrategy.setStatusTip('단순전략 생성')
        multiStrategy.setStatusTip('복합전략 생성')
        simpleStrategy.triggered.connect(self.simple_strategy_tab)

        getOrder = QAction('&불러오기', self)
        getOrder.setStatusTip('불러오기')
        #getOrder 전략 파일 불러오는 함수 연결

        saveOrder = QAction('&저장', self)
        saveOrder.setStatusTip('저장')

        saveNameOrder = QAction('&다른이름으로 저장', self)
        saveNameOrder.setStatusTip('다른이름으로 저장')

        newOrder.addAction(simpleStrategy)
        newOrder.addAction(multiStrategy)

        orderMenu.addMenu(newOrder)
        orderMenu.addAction(getOrder)
        orderMenu.addAction(saveOrder)
        orderMenu.addAction(saveNameOrder)
        ...
        # 종목필터링 메뉴
        filterMenu = mainMenu.addMenu('종목필터링')
        filter_Menu = QAction('&종목필터링', self)
        filter_Menu.setStatusTip('종목필터링')
    ...
    # 메뉴 선택 시 탭 생성 함수
    def chart_tab(self):
        chart_tab = stock_chart_menu.stock_chart()
        self.tabs.addTab(chart_tab, '종목차트')

    def simple_strategy_tab(self):
        simple_strategy_tab = simple_strategy_menu.simple_strategy()
        self.tabs.addTab(simple_strategy_tab, '단순전략')

    def basic_tab(self):
        basic_tab = basic_backtest_menu.basic_backtest()
        self.tabs.addTab(basic_tab, '기본백테스트')

    def label_tab(self):
        label_tab = label_backtest_menu.label_backtest()
        self.tabs.addTab(label_tab, '레이블백테스트')

    def comPreChart_tab(self):
        comPreChart_tab = comprehensive_chart_menu.comPreChart()
        self.tabs.addTab(comPreChart_tab, '종합차트')

    def filter_tab(self):
        filter_tab = stock_filtering_menu.filtering()
        self.tabs.addTab(filter_tab, '종목필터링')
